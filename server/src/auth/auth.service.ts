...
export class AuthService {
  ...
  constructor(
    ...
    this.url = `${SERVER_CONFIG.HTTP_PROTOCOL}://${SERVER_CONFIG.DOMAIN}:${SERVER_CONFIG.PORT}`;
  }
  ...
  async createToken(user: IUser): Promise<IToken> {
    ...
    const token: string = sign({
      sub: user.id,
    }, SERVER_CONFIG.JWT_KEY, { expiresIn });
  ...
  async requestTwitterRedirectUri(): Promise<{ redirect_uri: string } | any> {
    return new Promise((resolve: Function, reject: Function) => {
      post({
        ...
        const { oauthToken } = this.parseTwitterResponse(body);
        const redirect_uri: string = `${this.twitterConfig.login_dialog_uri}?oauthToken=${oauthToken}`;
  ...
  async twitterSignIn(oauthToken: string, oauthVerifier: string): Promise<any> {
    return new Promise((resolve: Function, reject: Function) => {
      post({
        ...
        const { oauthToken, oauthToken_secret, user_id } = this.parseTwitterResponse(body);
  ...
  async requestGoogleRedirectUri(): Promise<{ redirectUri: string } | any> {
    const queryParams: string[] = [
    ...
    const redirectUri: string = `${this.googleConfig.login_dialog_uri}?${queryParams.join('&')}`;
